...
Feature: Frontend filters
  ...
  Scenario: Check many filters hints
    ...
    And I check "Open,Cancelled,Shipped" in Order Status filter in "PastOrdersGrid"
    Then I should see "Order Status 3" in the "PastOrdersGrid" element
    And I check "Closed,Archived" in Order Status filter in "PastOrdersGrid"
    Then I should see "Order Status 5" in the "PastOrdersGrid" element
    ...
    Scenario: Check filter hints responsive
      ...
      And I should see "+4" in the "PastOrdersGrid" element
      Then should see filter hints in "PastOrdersGrid" frontend grid:
        | Open      |
        | Cancelled |
        | Shipped   |
        | Closed    |
        | Archived  |
      And I click on "Filter Hint Items Toggle" with title "+4" in element "PastOrdersGrid"
      Then should see filter hints in "PastOrdersGrid" frontend grid:
        | Open      |
        | Cancelled |
        | Shipped   |
      ...
      And I should see "+8" in the "PastOrdersGrid" element
      Then should see filter hints in "PastOrdersGrid" frontend grid:
        | Archived  |
      And I should see "Clear All" in the "PastOrdersGrid" element
      Then I click on "Filter Hint Items Toggle" with title "+8" in element "PastOrdersGrid"
      Then should see filter hints in "PastOrdersGrid" frontend grid:
        | Open      |
        | Cancelled |
        | Shipped   |
    ...
    Scenario: Check reset particular filters by Clear filter Button
      ...
      And I click "Order Status 5" in "PastOrdersGrid" element
